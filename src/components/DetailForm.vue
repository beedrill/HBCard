<template>
    <div class="q-pa-md text-subtitle1" style="font-family: font199">
        ...
        <q-input class="q-my-sm" outlined v-model="name" label="昵称" />
         ...
         <q-input v-if="longSignature" class="q-my-sm" outlined v-model="secondLineSignature" label="第二行签名" />
         <div><q-toggle color="deep-orange" label="长签名(可以写两行签名)" v-model="longSignature"></q-toggle></div>
         陪玩项目：
          <q-select
            filled
            v-model="games"
...
export default {
  ...
  data () {
    return {
      ...
      games: [],
      gameOptions: [
        '英雄联盟', 'PUBG', 'TFT', 'COD', 'CSGO', '彩虹6号', '逃离塔科夫', 'Steam', 'DOTA2', 'Valorant', '守望先锋', 'Apex', '手游', '魔兽', '其他端游', '陪聊', '唱歌'
      ],
      voice: '',
      signature: '',
      imageUrl: '',
  ...
  methods: {
    ...
    getData () {
      ...
      return {
        ...
        secondLineSignature: this.secondLineSignature,
        avatar: imageUrl,
        longSignature: this.longSignature
      }
    },
    preview () {
